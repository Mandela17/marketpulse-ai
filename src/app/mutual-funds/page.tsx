  ...
  const addFundFromSearch = async (schemeCode: number, schemeName: string) => {
    ...
    try {
      ...
      const newFund: FundData = {
        ...
        aum: data?.groww?.aum ? Number(data.groww.aum) : undefined,
        expenseRatio: data?.groww?.expenseRatio ? parseFloat(String(data.groww.expenseRatio)) : undefined,
                      ...
                      <div style={{
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10, marginBottom: 16 }}>
                          {[
                            ...
                            ...(fund.aum ? [{ label: 'AUM', value: `₹${Number(fund.aum).toLocaleString('en-IN', { maximumFractionDigits: 0 })} Cr`, icon: '📊' }] : []),
                            ...(fund.expenseRatio ? [{ label: 'Expense Ratio', value: `${parseFloat(String(fund.expenseRatio)).toFixed(2)}%`, icon: '💸' }] : []),
                            ...(fund.fundManager ? [{ label: 'Fund Manager', value: fund.fundManager, icon: '👤' }] : []),
                            ...(fund.rating ? [{ label: 'Rating', value: '⭐'.repeat(Math.min(5, Math.round(Number(fund.rating)))), icon: '🏆' }] : []),
